...
def accuracy(typed, reference):
    ...
    for i in range(len(typed_words)):
        if i < len(reference_words) and typed_words[i] == reference_words[i]:
            acc += 1
    return acc * 100.0 / len(typed_words)
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    "*** YOUR CODE HERE ***"
    minutes = elapsed / 60
    typed_words_len = len(typed) / 5
    return typed_words_len / minutes
    # END PROBLEM 4
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for i in range(len(valid_words)):
        if user_word == valid_words[i]:
            return user_word
        diff_len = diff_function(user_word, valid_words[i], limit)
        if diff_len < min_diff_len and diff_len <= limit:
            min_diff_len = diff_len
            valid_index = i
    if valid_index != -1:
        return valid_words[valid_index]
    else:
        return user_word
    # END PROBLEM 5
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    player_nums = len(times_per_player)
    words_nums = len(words)
    times = [[0 for _ in range(words_nums)] for _ in range(player_nums)]
    for i in range(player_nums):
        for j in range(words_nums):
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    ret_words = [[] for _ in range(len(player_indices))]
    for word_index in word_indices:
        fastest_player_num = 0
        for player_num in player_indices:
            if time(game, player_num, word_index) < time(
                game, fastest_player_num, word_index
            ):
                fastest_player_num = player_num
        ret_words[fastest_player_num].append(word_at(game, word_index))
    return ret_words
    # END PROBLEM 10

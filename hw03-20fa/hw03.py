...
def balanced(m):
    ...
    case1 = length(left_arm) * total_weight(left_end) == length(
        right_arm
    ) * total_weight(right_end)
    case2 = balanced(left_end) and balanced(right_end)
...
def has_path(t, word):
    ...
    if is_leaf(t):
        ...
        if label(t) != word[0]:
            return False
        for branch in branches(t):
            if len(word[1:]) == 0 or has_path(branch, word[1:]):
                return True
        return False
...
def quadratic(x, a, b, c):
    ...
    f_extreme = a * extreme_x * extreme_x + b * extreme_x + c
    if lower < extreme_x and upper > extreme_x:
        if a > 0:
            return interval(f_extreme, max(f_lower, f_upper))
        else:
            return interval(min(f_lower, f_upper), f_extreme)
    else:

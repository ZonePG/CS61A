...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    disx = get_lat(city_a) - get_lat(city_b)
    disy = get_lon(city_a) - get_lon(city_b)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    pos = make_city("", lat, lon)
    dis_a = distance(pos, city_a)
    dis_b = distance(pos, city_b)
    if dis_a < dis_b:
...
def riffle(deck):
    ...
    "*** YOUR CODE HERE ***"
    return [
        deck[i // 2] if i % 2 == 0 else deck[len(deck) // 2 + (i + 1) // 2 - 1]
...
def add_trees(t1, t2):
    ...
    if len1 == len2:
        ...
        branches_t1 = branches(t1) + [tree(0) for _ in range(len2 - len1)]

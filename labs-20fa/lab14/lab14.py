...
def prune_min(t):
    ...
    if len(t.branches) == 0:
        return
    min_branch = min(t.branches, key=lambda branch : branch.label)
    t.branches.clear()
    t.branches.append(min_branch)
...
def num_splits(s, d):
    ...
    def num_splits_helper(s, n):
        ...
        first, rest = s[0], s[1:]
        return num_splits_helper(rest, n + first) + num_splits_helper(rest, n - first)
...
def foldl(link, fn, z):
    ...
    if link is Link.empty:
        return z
    "*** YOUR CODE HERE ***"
    return foldl(link.rest, fn, fn(link.first, z))
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldl(lst, lambda first, z: Link(first, z), Link.empty)

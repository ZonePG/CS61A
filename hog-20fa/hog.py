...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    sum = 0
    dice_ret_may_one = False
    for _ in range(num_rolls):
        dice_ret = dice()
        if dice_ret == 1:
            dice_ret_may_one = True
        sum += dice_ret
    if dice_ret_may_one:
        return 1
    return sum
...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        return False
    max_gcd = 1
    for gcd in range(2, min(player_score, opponent_score) + 1):
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if who == 1:
            score = score1
        dis = score - last_score
        if dis > running_high:
            print(dis, "point(s)! The most yet for Player", who)
        return announce_highest(who, score, max(dis, running_high))
    return say
    # END PROBLEM 7
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    score_after_roll0dice = score + free_bacon(opponent_score)
    if extra_turn(score_after_roll0dice, opponent_score):
        return 0
    else:
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    return extra_turn_strategy(score=score, opponent_score=opponent_score, cutoff=8, num_rolls=6)  # Replace this statement
    # END PROBLEM 12
